...
ARCHITECTURE_PHASE3.md §10 (확장):
    p3_card(today_p3, effective_mode)                          # index.html 의 Phase 3 카드 (P2 카드 아래)
    render_sizing_report(summary_p3, summary_v1, summary_v0, out_html, charts)   # docs/sizing_p3.html ①~⑪
    render_regime_report(summary_p3, out_html, charts)                           # docs/regime_p3.html ①~⑩
    render_track_record(track_p3, summary_p3, out_html, charts)                  # docs/track_record.html (§8.4 × §8.5)
    charts_p3(oos_p3, backtest_p3, spy_close, summary_p3, ledger_df)             # 비중 경로·누적수익·낙폭·사다리 프론티어·
                                                                                 # 멤버 밴드·P_high·시대 AUC·구간·BSS 분포·경보·킬
    render_index 는 today["p3"] 가 있으면 p3_card 를 p2_card 아래에 끼워 넣고 세 페이지 링크를 단다.
    유효 배치 모드 = p2.deploy_mode == "tones" ∧ p3.deploy_mode == "tones" ∧ ¬kill (§8.3) — 리포트가 스스로 계산하고
    호출자 값과 AND 한다(올려 잡지 않는다). info_only 면 상태·톤·비중·D_max 사다리를 숨긴다(§15 stage 0).
    지평별 표시 규칙(§8.4)의 단일 원천은 mrl.track (horizon_stage·may_show·horizon_copy) 이다.
...
def render_index(today: dict, ledger_summary: dict, out_html: Path) -> None:
    ...
    links = ('<section class="panel"><h2>자료</h2><ul class="plain">'
             ...
             + ('<li><a href="sizing_p3.html">Phase 3 비중 규칙 (백테스트 표 · D_max 사다리 · 유지 조건)</a></li>'
                '<li><a href="regime_p3.html">Phase 3 국면 엔진·등록부 (HMM · 그림자 멤버 · 채택 검정)</a></li>'
                '<li><a href="track_record.html">Phase 3 트랙레코드 (라이브 vs 백테스트 · 경보 · 킬룰)</a></li>'
                if p3sec else "")
             + f'<li><a href="{_esc(SITE_URL)}">{_esc(SITE_URL)}</a></li></ul>'
             f'<div class="foot">생성 {_esc(updated)} · 이 페이지는 투자 조언이 아닙니다.</div></section>')
    _write_html(out_html, "오늘 판정 — market-risk-lab",
...
P3_INFO_ONLY_LABEL = "정보 제공 전용 — 톤·비중 제안 숨김"          # VALIDATION §7 문구 (§8.3 카드)
P3_INFO_NOT_SIZING = "정보(비중 제안 아님)"                        # §15 stage 0 — 비중 규칙 출력의 회색 라벨
P3_SHADOW_LABEL = "그림자 — 확률·비중·결정층·킬룰에 들어가지 않음"   # §10 5 국면 게이지 라벨
P3_HIGH_STATE_KO = "고변동 상태"                                   # §16 9 — '위기' 가 아니다
P3_LADDER_HIDDEN = ("예산 사다리는 유효 배치 모드에서만 표시합니다 — §8.3: info_only 에서 카드는 "
                    "상태·톤·비중·D_max 사다리를 숨긴다(장부는 계속 기록).")
P3_CARD_FOOTNOTE = ("이 비중은 사전 등록 규칙을 SPY 100% 슬리브에 적용한 것이다; 가족의 현금·채권은 모델 밖 · "
                    "투자 조언 아님")
# §16 1 — 모든 페이지 첫 줄
P3_FIRST_LINE = ("Phase 3 는 skill 을 더하지 않는다 — 어떤 후보도 보정 VIX 위에 측정 가능한 Brier 이득이 없다"
                 "(M3→p3 −0.0002, ens(M3,H) +0.0000, M4a 무이득). 가치는 비중 규칙·시나리오·구간·"
                 "트랙레코드·킬 장치이며, 이 페이지는 그것만 주장한다.")
# §6.5 (a)~(d) 의 카드용 축약 (§10 2)
P3_SIZING_SHORT = ("변동성을 맞추면 파라미터 0개 규칙이 p2 결정층과 같다 · 낙폭 통제의 값은 보유 대비 "
                   "−3.6pp/년(달력연도의 ~90% 에서 뒤진다)")
P3_MEMBER_KO = {"p2": "p2", "M1": "보정 VIX", "H": "HMM", "ens": "앙상블"}
P3_REASON_KO = {"init": "첫 기록", "escalation": "격상 — 즉시 하향", "weekly": "주간 점검", "hold": "유지",
                "input_missing": "입력 결측 — 비중 유지(1.0 복귀 없음)", "info_only": "정보 제공 전용"}
...
P3_PARAM_TABLE = (
    ...
    {"줄": "결정층", "객체": "밴드 1.5/1.2·2.5/2.0, dwell 5", "K_s": 0, "K_u": 0, "생산 확률에?": "—",
     "예산 줄": "Phase 2 상수 동결"},
    {"줄": "비중 규칙(§6)", "객체": "λ .94, seed 60, 바닥 .25/상한 1, k_slow 3.5·k_fast 2.0, 격자 {6..15}%, "
                                  "D_max .35, 격자 .05, 밴드 .10, 주간+격상, 배수 = TONE_EXPOSURE, 비용 5bp·현금 0",
     "K_s": 0, "K_u": 0, "생산 확률에?": "—", "예산 줄": "§6-b: 적합 0 필수. ~40 변형을 1993~2024 로 본 뒤 "
                                                      "선택 → post hoc, 재조정 금지"},
    {"줄": "시나리오·경보·킬룰", "객체": "구간·풀링 20·z·D1~D11 임계·36/8/60/12·블록 40·4,000·seed 0",
    ...
    {"줄": "앙상블 가중", "객체": "동일", "K_s": 0, "K_u": 0, "생산 확률에?": "—", "예산 줄": "상수"},
    {"줄": "예산 밖(Phase 2, 불변)", "객체": "HAR OLS 4 · v0 Platt 2", "K_s": "표시 전용", "K_u": 0,
     "생산 확률에?": "아니오", "예산 줄": "—"},
)
P3_PARAM_TABLE_NOTE = ("생산 확률에 닿는 적합값: 4(예산 안) + 0(Phase 3). 가족이 보는 어떤 숫자에든 닿는 적합값 "
                       "합계 26 (4 + 0 + 4 변동성 표시 + 2 + 2 + 12 그림자 + 2 v0), 그중 예산 안 4. "
                       "설계 자유도(파라미터가 아니라 선택): HMM 관측형 · Platt 특징 · 바닥 재클립 · 주간 리듬 · "
                       "k_slow 3.5 · 풀링 20 · 2단계 킬 해석 · 경보 임계.")

# §16 정직 문구·리스크 — 모든 Phase 3 페이지의 마지막 섹션
P3_HONESTY_ITEMS = [
    "Phase 3 는 skill 을 더하지 않는다. 어떤 후보도 보정 VIX 위에 측정 가능한 Brier 이득이 없다"
    "(M3→p3_A −0.0002, ens(M3,H) +0.0000, M4a 무이득, 폭/신용 −0.055). 가치는 비중 규칙·시나리오·구간·"
    "트랙레코드·킬 장치다.",
    "Phase 2 의 공식 결과가 Phase 3 의 전제다. model_p2.json 이 info_only 이면 유효 모드가 info_only 이므로 "
    "비중·상태 카드가 뜨지 않는다. 이 결정은 Phase 3 코드가 아니라 장부에서 내려진다.",
    "비중 규칙의 가격: σ_T 10% 에서 보유보다 ~3.6pp/년 낮고 달력연도의 ~90% 에서 뒤진다; 현금 0%(+0.6pp/년 "
    "미반영); 5bp 는 분기별로 늦게 실행하는 가족의 실제 마찰(중개·세금)을 과소평가한다.",
    "완만한 약세장 실패 모드: 2000~02 변동성 단독 −30.7~−34.6%(σ_T 10%); k_slow 3.5 는 한 에피소드로 보정한 "
    "상수다 — 예산은 보장이 아니다. 저변동 급락(2018-02, 2020-02 첫 주)은 규칙이 반응하기 전에 맞는다.",
    "바닥의 대가: 곱 뒤 0.25 재클립은 MaxDD 를 −17.9% 에서 −23.5% 로 올린다(변동성 단독과 같다). 상태 배수의 "
    "낙폭 기여는 바닥 없이만 나타나고 그 형태는 2008~09·2020 에 사실상 퇴장(비중 0.05~0.10)이었다.",
    "모든 비중 상수는 post hoc(λ, 격자, 밴드, 리듬, k_slow, 바닥 처리; ~40 변형). 유지 조건 (a)·(b) 는 근소해 "
    "공식 수치에서 뒤집힐 수 있다 — 그러면 카드가 숨겨지고 장부만 쌓인다.",
    "소표본은 영구 조건: 연 ~12 독립 창·~1 에피소드. 3·6개월 skill 은 잡음(10/90분위 ±0.8)이라 표시하지 "
    "않는다 — n_eff < 6 에 BSS 없음, 판정일 전 판정어 없음은 코드가 강제한다(§8.4).",
    "HMM: 라벨 스위칭·EM 국소해·지속성 0.98 이라 급락을 늦게 본다. 점유 43% 는 '위기' 가 아니라 "
    f"'{P3_HIGH_STATE_KO}'. 비지도 12개도 파라미터로 공개하며 생산 확률에는 들어가지 않는다.",
    "데이터 개정: Yahoo 조정 종가 재다운로드가 σ̂·P_high·비중 이력을 조금 바꾼다 → D10 이 불일치를 세고 D7 은 "
    "daily 를 실패시킨다; 장부는 기록이며 다시 쓰지 않는다.",
    "결합 위험: 열 이름과 spec_sha256·registry_sha·sizing_sha 가 계약이다. 불일치는 exit 1, 대체 없음. "
    "킬은 model_p2.json 을 건드리지 않고 유효 모드의 AND 로만 작동한다.",
    "홀드아웃: 2024-09-01~ 는 unlock 파일 없이 채점하지 않는다. 그림자 멤버의 홀드아웃 점수는 정보일 뿐 "
    "채택에 쓰지 않으며, 신선 블록은 라이브 시작부터만 센다.",
    "제품 위험: 확률·상태기계·변동성 규칙 세 겹은 v0 톤보다 설명이 어렵다 → 카드는 한 숫자·한 문장으로 "
    "시작한다. 시나리오 셀은 얇다(reduce n_eff 12, 에피소드 33/11/4) — 회색·풀링 규칙은 코드가 강제한다.",
    "계산: 주간 +< 4분, 일간 +< 5초; 새 의존성 없음(numpy EM).",
]

LABELS.update({
    # 비중(§6.3 백테스트 표 · §6.2 사다리)
    "row": "행", "window": "기간", "max_dd_date": "MaxDD 일자", "worst_month_label": "최악 월(월)",
    "d_cagr_vs_bh": "ΔCAGR vs 보유", "d_maxdd_vs_bh": "ΔMaxDD vs 보유", "vol_ratio": "실현변동성/σ_T",
    "maxdd_over_sigma_t": "MaxDD/σ_T", "sigma_target": "σ_T", "d_slow": "D_slow(3.5×)", "d_fast": "D_fast(2.0×)",
    "maxdd_vol_only": "MaxDD 변동성 단독(1993~)", "maxdd_vol_only_date": "일자(변동성 단독)",
    "maxdd_decision": "MaxDD 결정층 포함(2003~)", "maxdd_decision_date": "일자(결정층 포함)",
    "share_behind_years": "보유에 뒤진 달력연도 비율", "protection_pp": "보호(규칙−보유)",
    "ret_rule": "규칙 수익", "ret_bh": "보유 수익", "avg_exposure": "평균 비중", "w_exec": "실행 비중",
    "w_vol": "변동성 비중", "w_target": "목표 비중", "mult": "상태 배수", "cand": "격자 후보", "reason": "사유",
    "week_end": "주간 점검일", "sigma": "예상 변동성(EWMA)", "sigma_ewma": "예상 변동성(EWMA)",
    "sigma_har_fc": "예상 변동성(HAR, 표시 전용)", "sigma_down": "하향 임계", "sigma_up": "상향 임계",
    "next_check": "다음 점검일", "d_max": "예산 D_max", "deploy_sizing": "비중 블록 표시",
    # 유지 조건·상대성과
    ...
    # 등록부·HMM
    "member": "멤버", "K_s": "지도(K_s)", "K_u": "비지도(K_u)", "in_average": "생산 평균에", "status": "상태",
    "ledger_no": "장부 번호", "source": "출처", "admission": "채택 검정", "theta_id": "θ id",
    "p_high": "P(고변동)", "q20": "20세션 안 고변동 확률", "p_k": "k세션 뒤 고변동", "q_k": "k세션 안 고변동",
    "dwell": "기대 체류(세션)", "p00": "p00", "p11": "p11", "occupancy_high": "고변동 점유",
    "refit_date": "재적합일", "guard": "guard", "guard_ok": "guard 통과", "n_iter": "EM 반복",
    # 시나리오
    "bin_lo": "구간 하한", "bin_hi": "구간 상한", "pooled": "풀링", "ret_p10": "20일 수익 p10",
    "ret_p50": "20일 수익 p50", "ret_p90": "20일 수익 p90", "mdd_p10": "20일 최대낙폭 p10",
    "mdd_p50": "20일 최대낙폭 p50", "share_ep10_start": "≥10% 에피소드 시작 비율", "grey": "회색(단독 표시 금지)",
    "obs_live": "라이브 실현 비율", "n_live": "라이브 n", "n_eff_live": "라이브 n_eff",
    # 트랙레코드·경보·킬
    "code": "코드", "action": "효과", "asof": "기준일", "stage": "지평 단계", "live_start": "라이브 시작",
    "months": "경과(개월)", "months_elapsed": "경과(개월)", "episodes5": "실현 ≥5% 에피소드",
    "stage1_due": "1차 평가 도달", "stage2_due": "2차 평가 도달", "ci_label": "CI 라벨", "killed": "킬",
    "gross_failure_badge": "중대 실패 배지", "n_scored": "채점 행", "replay_mismatch_count": "D10 불일치 수",
    "hist_pct": "과거 동일 길이 창 백분위", "backtest": "백테스트 참조", "fresh_blocks": "신선 블록",
    "switches_per_year": "전환/년", "w_changes_per_year": "비중 변경/년", "n_changes_252": "252세션 변경 수",
    "share_gt_030": "p > 0.30 비율", "share_floor": "바닥(0.25) 비율", "share_full": "100% 비율",
    "avg_w": "평균 비중", "har_log_mae": "HAR log-MAE", "ewma_log_bias": "ln(실현/EWMA) 편향",
...
def _p3_table(records, order=None, empty_msg="자료 없음") -> str:
    return _records_table(records, order, empty_msg, fmt=_fmt_p3)


def _p3_kv(d, empty_msg="자료 없음") -> str:
    return _kv_table(d, empty_msg, fmt=_fmt_p3)
...
def _hz_val(hz, key: str, txt: str) -> str:
    ...
    if horizon_may_show(hz, key):
        return txt
    return f'<span class="mut">— <span class="note" style="display:inline">({_esc(_hz_reason(hz, key))})</span></span>'
...
def _hz_block(hz) -> str:
    ...
    parts = [f'<div class="note">지평 단계 <b>{_esc(_hz_stage(hz))}</b> · 경과 {_fmt(hz.get("months"), "months")}개월 · '
             f'n_eff {"—" if math.isnan(n_eff) else f"{n_eff:.1f}"}</div>']
    if sents:
        parts.append('<div class="quote">' + "<br>".join(_esc(s) for s in sents) + "</div>")
    if show:
        parts.append(f'<div class="note">보이는 것: {_esc(" · ".join(show))}</div>')
    if hide:
        parts.append(f'<div class="note">보이지 않는 것(코드가 막음): {_esc(" · ".join(hide))}'
                     + (f' — {_esc(hz.get("hidden_reason"))}' if hz.get("hidden_reason") else "") + "</div>")
    parts.append(f'<div class="note">{_esc(hz.get("footnote_ko") or _track.HORIZON_FOOTNOTE)}</div>')
    return "".join(parts)
...
def p3_param_line(d: dict | None = None) -> str:
    ...
    k_prod = int(_first(src, "k_prod", "param_count", default=P2["param_count"]) or P2["param_count"])
    ...
    return P3_PARAM_LINE_FMT.format(k_prod=k_prod, budget=budget, k_p3=k_p3, h_ks=h_ks, h_ku=h_ku)
...
def p3_verdict_block(acc: dict, *, tag: str = "배치 판정(Phase 2 acceptance) — 원문 그대로") -> str:
    """acceptance.verdict_line 을 **그대로** 싣는 블록. 없으면 조용히 넘기지 않고 그 사실을 적는다."""
    line = acceptance_verdict_line(acc if isinstance(acc, dict) else {})
    if not line:
        return ('<div class="quote"><b>배치 판정 줄 없음</b> — summary_p2.json:acceptance.verdict_line 을 '
                '찾지 못했습니다. 배치 여부를 리포트가 대신 판단하지 않습니다(조용한 실패 금지).</div>')
    dep = (acc or {}).get("deploy_mode")
    tone = (acc or {}).get("tone_model")
    tail = f"deploy {dep or '—'}" + (f" · 톤 모델 {tone}" if tone else " · 배포된 단 없음 — 톤·비중 주장 없음")
    return (f'<div class="quote"><b>{_esc(tag)}</b><br>{_esc(line)}'
            f'<div class="note">{_esc(tail)}</div></div>')
...
def _p3_mode_banner(eff: dict, deploy_sizing=None) -> str:
    ...
    if tones:
        body = (f'<b>유효 배치 모드: tones</b> — p2 {_esc(eff.get("p2") or "—")} ∧ p3 {_esc(eff.get("p3") or "—")} '
                "∧ 킬 없음")
        extra = ""
    else:
        body = (f'<b>{_esc(P3_INFO_ONLY_LABEL)}</b> (VALIDATION §7) — '
                + _esc(" · ".join(eff.get("reasons") or ["사유 미기록"])))
        extra = ('<div class="note">§15 stage 0 · §8.3: 상태·톤·비중·D_max 사다리를 숨기고, 변동성 단독 규칙·'
                 f'국면 게이지·시나리오·구간만 회색 \'{_esc(P3_INFO_NOT_SIZING)}\' 로 보여줍니다. '
                 '장부는 p·상태·w_vol 을 reason info_only 로 계속 기록합니다(가정치).</div>')
    if deploy_sizing is False:
        extra += ('<div class="note">유지 조건(§6.4) 위반 — deploy_sizing=false 이므로 비중 블록을 숨깁니다'
                  '(장부는 계속 기록).</div>')
...
def _p3_kill_bits(d: dict, hz: dict) -> list[str]:
    ...
    bits = [_esc(f"킬 카운트다운: 실현 ≥5% 에피소드 {ep or '—'} · 경과 {mo or '—'}개월({cap})")]
    sc = _sub(ks, "score")
    bss = _fnum(sc.get("bss_clim") if sc else ks.get("bss_clim"))
    ci = (sc.get("ci_bss_clim") if sc else ks.get("ci")) or ks.get("ci_bss_clim")
    skill = (f"현재 skill {_fmt_p2(bss, 'bss')}"
             + (f" (95% {_fmt_p2(ci, 'bss')})" if isinstance(ci, (list, tuple)) and len(ci) == 2 else "")
             if not math.isnan(bss) else "현재 skill — (채점된 행 없음)")
    bits.append(_hz_val(hz, "bss", _esc(skill)))
    hp = _sub(d, "track").get("hist_pct") or _sub(_sub(d, "track", "panel"), "vii_skill").get("hist_pct")
    if isinstance(hp, dict) and hp:
        txt = " · ".join(f"{k}세션 {_fmt_p3(v, 'hist_pct')}" for k, v in hp.items() if v is not None)
        if txt:
            bits.append(_hz_val(hz, "hist_pct", _esc(f"과거 동일 길이 창 대비 백분위 — {txt}")))
    st = ks.get("state")
    if st:
        bits.append(_hz_val(hz, "verdict", _esc(f"킬룰 상태: {st}")))
    if ks.get("gross_failure_badge"):
        bits.append(_esc("중대 실패 배지(BSS ≤ −0.05 ∧ CI 상한 ≤ 0) — 중간 점검 표시일 뿐 킬이 아니다"))
    bits.append(_esc(kill_power_line(d)))                 # 카운트다운을 정직하게 만드는 줄(§8.3)
    return bits
...
def p3_card(today_p3: dict, effective_mode: str | None = None) -> str:
    ...
    **유효 배치 모드가 info_only 면**(p2 info_only · p3 info_only · 킬 중 하나라도) 상태·톤·비중·D_max 사다리를
    ...
    # 머리
    tags = [f'<span class="tag{"" if tones else " warn"}">'
            + _esc("비중 적용" if tones else P3_INFO_NOT_SIZING) + "</span>"]
    if asof:
        tags.insert(0, f'<span class="tag">기준일 <b>{_esc(pd.Timestamp(asof).strftime("%Y-%m-%d"))}</b></span>')
    if not math.isnan(sT):
        tags.append(f'<span class="tag">목표 변동성 <b>{_pct1(sT, 0)}</b></span>')
    if d.get("deploy_sizing") is False:
        tags.append('<span class="tag bad">유지 조건 위반 — 비중 블록 숨김</span>')
    ...
    if halt:
        tags.append('<span class="tag bad">경보 정지 ' + _esc(", ".join(sorted({str(a.get("code")) for a in halt})))
                    + "</span>")
    head = ('<div class="eyebrow">' + _esc("Phase 3 · 비중 규칙(적합 파라미터 0) · 그림자 등록부 · 시나리오") + "</div>"
            "<h2>오늘의 주식 비중과 그 근거</h2>"
            f'<div class="tags">{"".join(tags)}</div>'
            f'<div class="note">{_esc(P3_FIRST_LINE)}</div>')

    # 1. 숫자 하나 + 문장 하나 (A·B) — info_only 면 회색 정보 블록
    vol_rule_txt = (f"변동성 단독 규칙 w_vol = clip(목표 {_pct1(sT, 0)} ÷ 예상 {_pct1(sigma)}, "
                    f"{P3['w_min']:.2f}, {P3['w_max']:.2f}) = {'—' if math.isnan(w_vol) else f'{w_vol:.2f}'}")
    if tones:
        mult_txt = ("—" if math.isnan(mult) else f"{mult:.2f}") + (f" ({_esc(state)})" if state else "")
        s1 = (f'<div class="verdict">오늘 주식 비중 {"—" if math.isnan(w_exec) else f"{w_exec:.2f}"}'
              f'</div><div class="v-act">— 변동성 규칙 {"—" if math.isnan(w_vol) else f"{w_vol:.2f}"} '
              f'(목표 {_pct1(sT, 0)} ÷ 예상 {_pct1(sigma)}) × 상태 배수 {mult_txt}</div>')
        line_b = []
        nc = thr.get("next_check")
        if nc:
            line_b.append(f"다음 점검 {_dow_ko(nc)}({_esc(str(nc))})")
        sd, su = _fnum(thr.get("sigma_down")), _fnum(thr.get("sigma_up"))
        if not math.isnan(w_exec) and not math.isnan(sd):
            line_b.append(f"예상 변동성이 {_pct1(sd)} 를 넘으면 {max(w_exec - band, float(P3['w_min'])):.2f}")
        elif thr.get("down_note"):
            line_b.append(str(thr["down_note"]))
        if not math.isnan(w_exec) and not math.isnan(su):
            line_b.append(f"{_pct1(su)} 아래면 {min(w_exec + band, float(P3['w_max'])):.2f}")
        elif thr.get("up_note"):
            line_b.append(str(thr["up_note"]))
        line_b.append("결정층이 격상되면 즉시 하향")
        s1 += f'<div class="note">{_esc(" · ".join(line_b))}</div>'
        if reason:
            changed = d.get("changed") if d.get("changed") is not None else sz.get("changed")
            s1 += (f'<div class="note">사유 {_esc(P3_REASON_KO.get(reason, reason))} ({_esc(reason)})'
                   + (f" — 직전 {_fmt_p3(sz.get('prev_w_exec'), 'prev_w_exec')} 에서 변경" if changed else "")
                   + "</div>")
    else:
        recorded = [_esc(vol_rule_txt + " — 상태 배수 미적용(장부 reason info_only, 가정치)")]
        if not math.isnan(sigma):
            recorded.append(_esc(f"예상 변동성(EWMA λ=0.94) {_pct1(sigma)}"))
        ks = _sub(d, "kill") or _sub(trk, "kill")
        sc = _sub(ks, "score")
        if sc.get("n"):
            nums = _esc(f"채점 {_fmt(sc.get('n'), 'n')}행 (독립 창 {_fmt_p3(sc.get('n_eff'), 'n_eff')})") + " · "
            nums += _hz_val(hz, "bss", _esc(f"BSS {_fmt_p2(sc.get('bss_clim'), 'bss')} "
                                            f"(95% {_fmt_p2(sc.get('ci_bss_clim'), 'bss')})"))
            recorded.append(nums)
        if ks.get("episodes5") is not None:
            recorded.append(_esc(f"실현 ≥5% 에피소드 {_fmt(ks.get('episodes5'), 'n')}회 · 경과 "
                                 f"{_fmt(ks.get('months'), 'n')}개월"))
        s1 = (f'<div class="info"><div class="verdict" style="color:{PALETTE["mut"]}">{_esc(P3_INFO_ONLY_LABEL)}</div>'
              f'<div class="tags"><span class="tag warn">{_esc(P3_INFO_NOT_SIZING)}</span>'
              f'<span class="tag warn">{_esc(INFO_DISPLAY_LABEL)}</span></div>'
              + "".join(f'<div class="note">{x}</div>' for x in recorded)
              + f'<div class="note">사유: {_esc(" · ".join(eff["reasons"] or ["사유 미기록"]))}</div>'
              + '<div class="note">비중·상태·톤을 제안하지 않습니다. 이 숫자는 규칙이 무엇을 하고 있는지 보여주는 '
                '정보이며, 가족의 배분은 v0 판정을 따릅니다.</div></div>')
    ...
    if tones:
        ...
        s2 = ("<h3>예산 줄</h3>"
              + (f'<div class="quote">{_esc(str(sent))}</div>' if sent else
                 '<div class="note">예산 문장 없음 — d_max·σ_T 가 없습니다</div>')
              + f'<div class="note">{_esc(bd.get("short") or P3_SIZING_SHORT)} · '
                '<a href="sizing_p3.html">D_max 사다리·백테스트 표</a></div>')
    else:
        s2 = f'<h3>예산 줄</h3><div class="note">{_esc(P3_LADDER_HIDDEN)}</div>'
    ...
    for name in ("p2", "M1", "H"):
        ...
        parts.append(f"{lab} {int(round(v * 100))}")
    if math.isnan(lo) or math.isnan(hi):
        s3 = ('<h3>확률 헤드라인 — 모델 간 폭</h3>'
              '<div class="note">불일치 구간 없음(멤버 확률이 기록되지 않았습니다) — 조용히 좁히지 않습니다</div>')
    else:
        wpp = (hi - lo) * 100.0
        s3 = ('<h3>확률 헤드라인 — 모델 간 폭</h3>'
              f'<div class="v-act">모델들은 100일 중 {int(round(lo * 100))}~{int(round(hi * 100))}일로 갈린다'
              + (f' ({_esc(" · ".join(parts))}; 불일치 {wpp:.1f}pp)' if parts else f" (불일치 {wpp:.1f}pp)")
              + "</div>")
        if dis.get("flag"):
            s3 += (f'<div class="tag bad">불일치 플래그 — 폭 &gt; {ENSEMBLE_P3["disagree_flag"]["width"] * 100:.0f}pp 가 '
                   f'{ENSEMBLE_P3["disagree_flag"]["sessions"]}세션 이상 이어짐</div>')
        # 배포된 단이 없으면 '배포 단' 이라 쓰지 않는다 — 같은 페이지가 '배포된 단 없음' 이라 말하고 있다
        s3 += ('<div class="note">구간은 표시 전용입니다. 그림자 멤버는 생산 확률에 들어가지 않으며'
               + (f'(오늘 배포 단 {_esc(rung)})' if dep3["deployed"]
                  else f'(오늘 생산 확률 단 {_esc(rung)} — {_esc(INFO_DISPLAY_LABEL)})')
               + f', 구간 출처 {_esc(str(dis.get("src") or "—"))}.</div>')
    ...
    s4 = "<h3>시나리오 (세기만 한다 — 적합 0)</h3>"
    if lines:
        s4 += '<ul class="plain">' + "".join(f"<li>{_esc(x)}</li>" for x in lines) + "</ul>"
    else:
        s4 += '<div class="note">시나리오 표 없음 — n·n_eff·구간 없이는 문장을 만들지 않습니다</div>'
    if dd:
        ddv = _fnum(dd.get("dd_from_ath"))
        s4 += (f'<div class="note">ATH 대비 현재 {_pct1(ddv)}'
               + (f' (고점 {_esc(str(dd.get("ath_date")))})' if dd.get("ath_date") else "")
               + (f" · −5% 돌파 뒤 {_fmt(dd.get('sessions_since_breach'), 'n')}세션"
                  if dd.get("breached_5") and dd.get("sessions_since_breach") is not None else "")
               + "</div>")
    s4 += f'<div class="note">{_esc(scen.get("spread_sentence") or _scenarios_spread_sentence())}</div>'
    for n in (scen.get("notes") or []):
        s4 += f'<div class="note">{_esc(str(n))}</div>'
    ...
    if hide_gauge:
        s5 = ('<h3>국면 게이지 (그림자)</h3><div class="note">게이지 숨김 — D9(후행 756세션 P_high AUC &lt; 0.5) '
              '또는 guard 실패. 그림자 멤버는 rejected 로 기록됩니다.</div>')
    elif math.isnan(p_high):
        s5 = '<h3>국면 게이지 (그림자)</h3><div class="note">P(고변동) 없음 — HMM 값이 기록되지 않았습니다</div>'
    else:
        lvl = P3_REGIME_LEVELS[0] if p_high < P3_REGIME_CUTS[0] else (
            P3_REGIME_LEVELS[1] if p_high < P3_REGIME_CUTS[1] else P3_REGIME_LEVELS[2])
        dwell = _fnum(rg.get("dwell") if rg.get("dwell") is not None else rg.get("expected_dwell"))
        s5 = (f'<h3>국면 게이지 (그림자)</h3><div class="v-act">{_esc(lvl)} — '
              f'P({_esc(P3_HIGH_STATE_KO)}) {_pct1(p_high)}'
              + (f' · {int(ks20.get("k") or P2["h"])}세션 안 {_esc(P3_HIGH_STATE_KO)} 확률 {_pct1(q20)}'
                 if not math.isnan(q20) else "")
              + (f" · 기대 체류 {dwell:.0f}세션" if not math.isnan(dwell) else "") + "</div>"
              f'<div class="note">{_esc(P3_SHADOW_LABEL)} · 점유는 \'위기\' 가 아니라 '
              f'\'{_esc(P3_HIGH_STATE_KO)}\' 입니다 (§16 9)</div>')

    # 6. 정직 스트립 — items 는 이미 HTML(필요한 곳만 _esc)
    items = [_esc(p3_param_line(d))]
    ...
    items.append(_esc(" · ".join(shas)))
    items.append(_esc(f"유효 모드(p2 ∧ p3 ∧ ¬kill) = {eff['mode']} — p2 {eff.get('p2') or '확인 불가'} · "
                      f"p3 {eff.get('p3') or '확인 불가'} · 킬 {'있음' if eff['killed'] else '없음'}"
                      + (f" ({'; '.join(eff['reasons'])})" if eff["reasons"] else "")))
    items.extend(_p3_kill_bits(d, hz))
    items.append(_esc(f"지평({_hz_stage(hz)}): {hz.get('sentence_ko') or ''}"))
    if al:
        items.append(_esc("경보 " + " · ".join(f"{a.get('code')}({a.get('action') or '표시'})" for a in al)))
    else:
        items.append(_esc("경보 없음(D1~D11)"))
    rmc = trk.get("replay_mismatch_count", d.get("replay_mismatch_count"))
    items.append(_esc(f"D10 재현 불일치 {'—' if rmc is None else _fmt(rmc, 'n')}건"))
    fb = trk.get("fresh_blocks") or d.get("fresh_blocks")
    if isinstance(fb, dict):
        items.append(_esc(f"신선 블록 {_fmt(fb.get('n_complete'), 'n')}/"
                          f"{fb.get('required') or ENSEMBLE_P3['fresh_blocks_min']}"))
    else:
        items.append(_esc(f"신선 블록 —/{ENSEMBLE_P3['fresh_blocks_min']} (라이브 시작 전에는 세지 않는다)"))
    acc_line = acceptance_verdict_line(p2_acceptance_of(d))
    if acc_line:
        items.append(_esc(f"배치 판정(Phase 2, 원문 그대로) — {acc_line}"))
    strip = ('<div class="strip"><b>정직 스트립</b><ul class="plain">'
             + "".join(f"<li>{x}</li>" for x in items)
             + f'</ul><div class="note">{_esc(P3_CARD_FOOTNOTE)}</div>'
             + f'<div class="note">{_esc(hz.get("footnote_ko") or _track.HORIZON_FOOTNOTE)}</div></div>')
    wsec = ("<h3>P3 경고</h3>" + _warn_list(warns)) if warns else ""
...
# Phase 3 페이지 공통 조각
# ------------------------------------------------------------------
def _p3_head(eyebrow: str, title: str, sub: str, tags: list[str], acc: dict, eff: dict,
             deploy_sizing=None) -> str:
    """모든 Phase 3 페이지의 머리: §16 1 첫 줄 → 배치 판정(원문) → 유효 모드 배너."""
    return ("<header>"
            f'<div class="eyebrow">{_esc(eyebrow)}</div><h1>{_esc(title)}</h1>'
            f'<div class="sub">{_esc(sub)}</div>'
            + (f'<div class="tags">{"".join(tags)}</div>' if tags else "")
            + f'<div class="quote">{_esc(P3_FIRST_LINE)}</div>'
            + p3_verdict_block(acc)
            + _p3_mode_banner(eff, deploy_sizing)
            + "</header>")


def _p3_nav(titles: list[str]) -> str:
    return '<nav class="nav">' + "".join(f'<a href="#s{i + 1}">{_esc(t)}</a>' for i, t in enumerate(titles)) + "</nav>"
...
def kill_power_line(src, member: str = "p2") -> str:
    ...
    if not isinstance(kp, dict) or not kp.get("sentence_ko"):
        return ("킬룰 검정력: 이번 주간 자기검사(kill_replay)가 이 수치를 재생성하지 않아 표시하지 않는다"
                "(§16.7 — 재생성되지 않으면 페이지에 쓰지 않는다).")
    return (f"킬룰 검정력({kp.get('p_col') or member}, 주간 자기검사 재생성 · 창 {_fmt(kp.get('n_windows'), 'n')}): "
            f"{kp['sentence_ko']} 2단계 해석은 문자 그대로보다 엄격하다(#7).")


def _p3_honesty_section(sec_id: str, extra: list[str] | None = None, *, kill_power_text: str | None = None) -> str:
    items = list(P3_HONESTY_ITEMS)
    if kill_power_text:                                   # 지운 정적 문장이 있던 자리에 산출값을 넣는다
        items.insert(6, str(kill_power_text))
    items += [str(x) for x in (extra or [])]
    return (f'<section class="panel" id="{_esc(sec_id)}"><h2>정직 문구와 리스크 (§16)</h2>'
            '<ul class="plain">' + "".join(f"<li>{_esc(x)}</li>" for x in items) + "</ul>"
            f'<div class="note">{_esc(P3_CARD_FOOTNOTE)}</div></section>')


def _panel_kv(block, live_label: str = "라이브", ref_label: str = "백테스트 참조") -> str:
    ...
    rows = "".join(f"<tr><td>{_label(k)}</td><td class=\"num\">{_fmt_p3(v, k)}</td>"
                   f"<td class=\"num\">{_fmt_p3(btd.get(k), k) if k in btd else '—'}</td></tr>"
                   for k, v in flat.items())
    out = ""
    if rows:
        out += ('<div class="tblwrap"><table><thead><tr><th>항목</th>'
                f'<th class="num">{_esc(live_label)}</th><th class="num">{_esc(ref_label)}</th>'
                f"</tr></thead><tbody>{rows}</tbody></table></div>")
    if not isinstance(bt, dict) and bt is not None:
        out += f'<div class="note">{_esc(ref_label)}: {_fmt_p3(bt, "backtest")}</div>'
    for k, v in nested.items():
        recs = _to_records(v)
        if recs:
            out += f"<h3>{_label(k)}</h3>" + _p3_table(recs)
        elif isinstance(v, dict) and v:
            out += f"<h3>{_label(k)}</h3>" + _p3_kv(v)
    if hidden:
        out += (f'<div class="note">숨김: {_esc(", ".join(str(x) for x in hidden))}'
                + (f" — {_esc(str(reason))}" if reason else "") + "</div>")
    return out or '<div class="note">자료 없음</div>'


def _p3_window_tables(records: list[dict], order: list[str], empty: str) -> str:
    """행에 'window' 가 있으면 창별로 나눠 §6.3 형식을 유지한다."""
    if not records:
        return f'<div class="note">{_esc(empty)}</div>'
    wins: list[str] = []
    for r in records:
    ...
    for w in wins:
        ...
        out.append(f"<h3>{_esc(w or '창 미상')}</h3>" + _p3_table(rows, order, empty))
    return "".join(out)
...
# ------------------------------------------------------------------
_SIZING_SECTIONS = ["① v0 요약(영구)", "② p2 결정층", "③ 정직한 읽기", "④ 백테스트 표", "⑤ D_max 사다리",
                    "⑥ 민감도", "⑦ 에피소드 손익", "⑧ 유지 조건", "⑨ 창별 상대성과", "⑩ 규약·해시", "⑪ 정직 문구"]
...
def render_sizing_report(summary_p3: dict, summary_v1: dict, summary_v0: dict, out_html: Path,
    ...
    tags = [f'<span class="tag">σ_T <b>{_pct1(sT, 0)}</b></span>',
            f'<span class="tag">예산 D_max <b>{_pct1(d_max, 0)}</b></span>',
            f'<span class="tag">sizing_sha <b class="mono">{_esc(str(run.get("sizing_sha") or "—")[:12])}</b></span>',
            f'<span class="tag">생성 <b>{_esc(run.get("generated_at_utc") or _now_str())}</b></span>',
            f'<span class="tag{"" if deploy_sizing else " warn"}">deploy_sizing <b>'
            f'{_fmt(deploy_sizing, "deploy_sizing")}</b></span>']
    if warns:
        tags.append(f'<span class="tag warn">경고 {len(warns)}건</span>')
    head = _p3_head("market-risk-lab · Phase 3 · 비중(sizing) — 적합 파라미터 0",
                    "가족 위험예산 → 변동성 목표 × 상태 배수",
                    "σ̂ = EWMA(λ=0.94), σ_T = D_max/3.5 를 격자로 내림, w = clip(clip(σ_T/σ̂, .25, 1) × 상태 배수, .25, 1). "
                    "5pp 격자·10pp 무거래 밴드·주 마지막 세션 갱신 + 결정층 격상 시 즉시 하향. 비용 5bp, 현금 0%. "
                    "적합 파라미터는 0개이며, 상수는 1993~2024 기록을 보고 정했으므로 그 기록에 대해 post hoc 입니다.",
    ...
    s1 = ('<section class="panel" id="s1"><h2>① v0 completed 요약 (영구 표기)</h2>'
          + _v0_line_html(v0_headline(v0src)) + "</section>")
    ...
    s2 = ('<section class="panel" id="s2"><h2>② p2 결정층 (같은 페이지의 가벼운 대안)</h2>'
          '<div class="note">결정층은 평균 비중 ~89% 로 보유에 가깝고, 비중 규칙은 ~69% 로 낙폭을 더 줄이는 대신 '
          'CAGR 을 더 내놓습니다. 가족은 사다리에서 더 높은 예산 행을 고를 수 있습니다(§16 3).</div>'
          + (_p3_table(p2row, _P3_BT_ORDER) if p2row else
             _p3_kv({k: _sub(v1, "allocation").get(k) for k in _ALLOC_KEYS} if _sub(v1, "allocation") else {},
                    "p2 결정층 행 없음 — summary_p3.sizing.table 또는 summary_v1.allocation 필요"))
          + "</section>")
    ...
    s3 = ('<section class="panel" id="s3"><h2>③ 정직한 읽기 (§6.5) — 표 위에 그대로</h2>'
          '<div class="quote">' + "<br><br>".join(_esc(str(x)) for x in hr) + "</div></section>")

    # ④ 백테스트 표 (세 창)
    s4 = ('<section class="panel" id="s4"><h2>④ 백테스트 성적표 (§6.3 형식 고정)</h2>'
          '<div class="note">규약 = evaluate.allocation_sim 그대로(w[t] → r[t+1], 5bp × |Δw|, 현금 0%). '
          '항상 보유·v0 completed(2015+)·p2 결정층·변동성 일치 참조(VT14) 행을 함께 싣습니다.</div>'
          + _p3_window_tables(tbl, _P3_BT_ORDER, "sizing.table 없음")
          + _img(charts.pop("cum_returns", None),
                 "누적수익(로그) — 규칙 vs 보유 vs p2 결정층 vs v0. 규칙은 대부분의 해에 보유에 뒤진다.",
                 "Cumulative growth: rule vs hold vs p2 vs v0")
          + _img(charts.pop("drawdown", None), "낙폭 비교 — 규칙 vs 보유. 상품은 낙폭 통제다.", "Drawdown comparison")
          + _img(charts.pop("w_path", None), "비중 경로와 SPY — 바닥 0.25, 상한 1.00.", "Exposure path vs SPY")
          + "</section>")

    # ⑤ D_max 사다리
    lad = _to_records(sz.get("ladder"))
    s5 = ('<section class="panel" id="s5"><h2>⑤ D_max 사다리 — 같은 σ_T 를 두 예산선으로</h2>'
          '<div class="note">D_slow = 3.5×σ_T (완만한 약세장선, 헤드라인) · D_fast = 2.0×σ_T (급락형 낙관선, 병기). '
          'MaxDD 는 1993~ 변동성 단독과 2003~ 결정층 포함 두 창을 나란히 둡니다. '
          '<b>예산은 보장이 아닙니다</b> — k_slow 3.5 는 2000~02 한 에피소드로 보정한 상수입니다(§16 4).</div>'
          + _p3_table(lad, _P3_LADDER_ORDER, "sizing.ladder 없음")
          + _img(charts.pop("ladder_frontier", None),
                 "MaxDD-vs-σ_T 프론티어(두 창)와 두 예산선 — 예산선 아래로 내려가면 예산 초과.",
                 "MaxDD vs sigma_T frontier")
          + (f'<div class="note">기본 예산 {_pct1(d_max, 0)} → σ_T {_pct1(sT, 0)} · 소유자는 가구 기본 D_max 를 '
             '장부 항목으로 등록하고, 다른 가족은 자기 행을 읽습니다.</div>')
          + "</section>")

    # ⑥ 민감도 (선택에 쓰지 않음)
    sens = _to_records(sz.get("sensitivities"))
    s6 = ('<section class="panel" id="s6"><h2>⑥ 민감도 (보고 전용 — 선택에 쓰지 않음)</h2>'
          '<div class="note">바닥 없음(B 원안)·월간·완만한 배수·0.25 격자·VT14·즉시 격상 없음·1세션 지연·10bp·'
          'S-HAR·일간 5pp·목표 격자. 합리적 변형 사이의 차이는 작아 \'규칙의 가족\' 으로 읽고 재조정하지 않습니다'
          '(§16 6, 장부 #6a).</div>'
          + _p3_window_tables(sens, _P3_BT_ORDER, "sizing.sensitivities 없음") + "</section>")

    # ⑦ 에피소드 손익
    ep = _to_records(sz.get("episode_pnl"))
    s7 = ('<section class="panel" id="s7"><h2>⑦ 에피소드 손익 (규칙 vs 보유, 구간 평균 비중)</h2>'
          '<div class="note">돌파일 결정 상태는 대부분 normal 입니다 — 모델이 첫날을 맞추리라 기대하지 마세요. '
          '저변동 급락(2018-02, 2020-02 첫 주)에는 보호가 거의 없습니다.</div>'
          + _p3_table(ep, ["peak_date", "trough_date", "depth", "ret_rule", "ret_bh", "protection_pp",
                           "avg_exposure", "n_sessions"], "sizing.episode_pnl 없음") + "</section>")
    ...
    s8 = ('<section class="panel" id="s8"><h2>⑧ 유지 조건 (§6.4; 공식 산출로 판정)</h2>'
          + _p3_table(crows, ["condition", "value", "threshold", "pass", "margin", "note"], "retention 없음")
          + (f'<div class="note">{_esc(str(ret.get("note_ko")))}</div>' if ret.get("note_ko") else "")
          + (f'<div class="tag bad">근소 통과 {_esc(", ".join(str(x) for x in ret.get("narrow") or []))} — '
             f'여유 {_pct1(ret.get("narrow_margin") or _sizing.NARROW_MARGIN, 0)} 미만, 공식 수치에서 뒤집힐 수 있습니다</div>'
             if ret.get("narrow") else "")
          + '<div class="note">라이브: D4(60세션 실현변동성/σ_T &gt; 1.5)·D4b(라이브 MaxDD &lt; −D_max)·'
            'D6(변경 &gt; 24/252세션)은 소유자가 장부 항목을 쓸 때까지 비중 카드를 정지합니다. '
            '§7 킬 스위치는 비중 성적표와 무관하게 상태 배수·비중 제안을 숨깁니다.</div>'
          + "</section>")
    ...
    s9 = ('<section class="panel" id="s9"><h2>⑨ 63/126/252 세션 창의 규칙 − 보유 (비겹침)</h2>'
          f'<div class="note">상품은 낙폭 통제입니다. 63/126/252 세션 창에서 뒤질 확률은 각각 {_sb} 이며, '
          f'달력연도의 {_cyb} 에서 뒤집니다 — 이 줄을 카드에서도 지우지 않습니다(§16 3).</div>'
          + _p3_table(rel_rows, ["L", "n", "p10", "p50", "p90", "share_behind", "worst", "best"],
                      "sizing.relative 없음")
          + (f'<div class="note">달력연도: {_fmt(cy.get("n_behind"), "n")}/{_fmt(cy.get("n_years"), "n")}년 뒤짐 '
             f'(중앙 {_pp1(cy.get("median"))} · 10/90 {_pp1(cy.get("p10"))}/{_pp1(cy.get("p90"))})</div>' if cy else "")
          + "</section>")
    ...
    s10 = ('<section class="panel" id="s10"><h2>⑩ 규약과 해시</h2>'
           '<div class="note">점 원칙: w_exec 는 t 종가에 확정되어 t+1 수익률에 적용됩니다("내일부터 주식 비중 X%"). '
           '비용 5bp × |Δw|(변경일에만), 현금 수익 0%(보수적 — 평균 현금 ~31% 에 2% 단기금리면 +0.6pp/년).</div>'
           + _p3_kv(conv)
           + f'<div class="note">규칙 문자열: <span class="mono">{_esc(_sizing.SIZING_RULE)}</span></div>'
           + f'<div class="note">적합 파라미터 수 = {_sizing.n_params()} (§6-b: 반드시 0) · {_esc(p3_param_line(s))}</div>'
           + ("<h3>경고</h3>" + _warn_list(warns) if warns else "")
           + "</section>")

    body = (head + nav + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9 + s10
            + _p3_honesty_section("s11", kill_power_text=kill_power_line(s)))
    _write_html(out_html, "비중 규칙 (Phase 3) — market-risk-lab", body)


# ------------------------------------------------------------------
# 주간 리포트 — 국면·등록부 (docs/regime_p3.html)  §10 주간 ①~⑨
# ------------------------------------------------------------------
_REGIME_SECTIONS = ["① 등록부·파라미터 회계", "② 사다리 확장", "③ 블록 표", "④ θ 경로·guard", "⑤ P_high",
                    "⑥ 시대별 AUC", "⑦ 소거", "⑧ 신뢰도", "⑨ 결정론·PIT·spec", "⑩ 정직 문구"]
...
def render_regime_report(summary_p3: dict, out_html: Path, charts: dict[str, bytes]) -> None:
    ...
    tags = [f'<span class="tag">registry_sha <b class="mono">{_esc(str(run.get("registry_sha") or "—")[:12])}</b></span>',
            ...
            f'<span class="tag">홀드아웃 <b>{_esc(HOLDOUT_START)}~ 하드컷</b></span>',
            f'<span class="tag">생성 <b>{_esc(run.get("generated_at_utc") or _now_str())}</b></span>']
    if warns:
        tags.append(f'<span class="tag warn">경고 {len(warns)}건</span>')
    head = _p3_head("market-risk-lab · Phase 3 · 국면 엔진(HMM) · 등록부",
                    "2-상태 가우시안 HMM(비지도)과 그림자 멤버 등록부",
                    "HMM θ 는 라벨을 보지 않고 종가만으로 추정하며(K_u 12), 전방 필터 P(고변동)의 로짓을 Platt 2개로 "
                    "확률에 사상한 것이 멤버 H 입니다. 오늘의 생산 확률은 Phase 2 그대로이고 등록 멤버는 전부 "
                    "그림자입니다 — 승격은 §6-c(d) 신선 자료 채택으로만 일어납니다.",
                    tags, acc, eff)
    ...
    s1 = ('<section class="panel" id="s1"><h2>① 등록부 (동결 튜플 — 변경은 번호 붙인 장부 항목으로만)</h2>'
          + _p3_table(reg, _P3_REGISTRY_ORDER, "registry 없음")
          + "<h3>채택 검정 (관측 기록은 기각만; 채택은 신선 자료만)</h3>"
          + (_p3_table(_to_records(adm), None, "admission 없음") if _to_records(adm) else _p3_kv(adm, "admission 없음"))
          + ("<h3>검정력 표</h3>" + _p3_table(pw) if pw else "")
          + (f'<div class="note">신선 블록 진행: {_esc(str(fb))}</div>' if fb else
             f'<div class="note">신선 블록: 라이브 시작 전에는 세지 않습니다(필요 {ENSEMBLE_P3["fresh_blocks_min"]}개, '
             f'{ENSEMBLE_P3["fresh_block_months"]}달력월 타일).</div>')
          + "<h3>파라미터 회계 (§14 — 이 표를 그대로)</h3>"
          + _records_table(list(P3_PARAM_TABLE), ["줄", "객체", "K_s", "K_u", "생산 확률에?", "예산 줄"])
          + f'<div class="note">{_esc(P3_PARAM_TABLE_NOTE)}</div>'
          + f'<div class="note">{_esc(p3_param_line(s))}</div>'
          + "</section>")

    # ② 사다리 확장
    lad = _ladder_records(s.get("ladder"))
    s2 = ('<section class="panel" id="s2"><h2>② 사다리 확장 (M1→H · M3→H · M3→ens · M3→M4a · B1→H)</h2>'
          '<div class="note">손실차 = (p_from−y)² − (p_to−y)² 의 평균(×1e-4, 양수 = 위 단이 좋음), 40일 블록 '
          '부트스트랩 4,000회 95% 구간, DM t(HAC 19), 위상 오프셋 부분표본. 어떤 후보도 측정 가능한 이득이 없습니다.</div>'
          + _p2_table(lad, _LADDER_ORDER, "ladder 없음") + "</section>")

    # ③ 블록 표
    blk = _sub(s, "blocks")
    b24 = _to_records(blk.get("24") or s.get("blocks24"))
    b18 = _to_records(blk.get("18") or s.get("blocks18"))
    s3 = ('<section class="panel" id="s3"><h2>③ 블록 표 (24개월 · 18개월)</h2>'
          '<div class="note">모든 표본 수는 n_blocks(= n/20) 를 병기합니다 — 5,453 을 \'관측치\' 로 읽지 마세요.</div>'
          + "<h3>24개월</h3>" + _p2_table(b24, _BLOCK_ORDER, "blocks24 없음")
          + "<h3>18개월</h3>" + _p2_table(b18, _BLOCK_ORDER, "blocks18 없음") + "</section>")
    ...
    s4 = ('<section class="panel" id="s4"><h2>④ θ 경로 · guard 로그 · 체류</h2>'
          '<div class="note">1월 첫 거래일 연 1회 재적합, 20세션 퍼지, warm start, 결정적 초기화. 상태 1 = 큰 수익률 '
          '분산(라벨 고정). 퇴화 θ(지속성 0.999+, 점유 1% 미만)는 guard 가 막고 이전 θ 를 유지합니다.</div>'
          + _p3_table(th, ["refit_date", "theta_id", "p00", "p11", "occupancy_high", "n_iter", "guard_ok",
                           "train_start", "train_end"], "hmm.theta_table 없음")
          + ("<h3>guard 로그</h3>" + _p3_table(gl) if gl else '<div class="note">guard 로그 없음</div>')
          + (f'<div class="note">계산 시간: {_esc(", ".join(f"{k} {_fmt_p3(v, k)}" for k, v in tm.items()))}</div>'
             if tm else "")
          + "</section>")

    # ⑤ P_high 밴드
    s5 = ('<section class="panel" id="s5"><h2>⑤ P(고변동) 밴드와 멤버 확률</h2>'
          f'<div class="note">{_esc(P3_SHADOW_LABEL)} · 점유 ~43% 는 \'위기\' 가 아니라 '
          f'\'{_esc(P3_HIGH_STATE_KO)}\' 입니다(§16 9). 지속성 0.98 이라 급락을 늦게 봅니다.</div>'
          + _img(charts.pop("hmm_phigh", None), "전방 필터 P(고변동)과 재적합 경계 — 평활(smoothed)이 아니라 필터입니다.",
                 "HMM filtered P(high) path")
          + _img(charts.pop("members_band", None), "멤버 확률과 불일치 밴드 vs SPY — 밴드는 표시 전용입니다.",
                 "Member probabilities and disagreement band")
          + "</section>")

    # ⑥ 시대별 AUC
    era = _to_records(s.get("era_auc"))
    s6 = ('<section class="panel" id="s6"><h2>⑥ 시대별 AUC (P_high vs p_vix vs M3)</h2>'
          '<div class="note">AUC 0.697 이 x_vix 보다 커 보여도 ~270 독립 창의 한 숫자이고 같은 정보'
          '(실현변동성 지속성)입니다 — Brier 이득은 0 입니다.</div>'
          + _p2_table(era, _ERA_ORDER, "era_auc 없음")
          + _img(charts.pop("era_auc", None), "시대별 AUC — 2020~24 의 VIX 판별력 감쇠와 HMM 의 보완.", "Era AUC bars")
          + "</section>")

    # ⑦ 소거
    abl = s.get("ablations")
    s7 = ('<section class="panel" id="s7"><h2>⑦ 소거 (q20 Platt · A 관측형 · B 동결형 · 완전/대각 공분산)</h2>'
          + (_p2_table(_to_records(abl), None, "ablations 없음") if _to_records(abl) else _p2_kv(abl, "ablations 없음"))
          + "</section>")
    ...
    s8 = ('<section class="panel" id="s8"><h2>⑧ 신뢰도 (H · 앙상블)</h2>'
          + "<h3>멤버 H</h3>" + _p2_table(rel_h, _RELIAB_ORDER, "reliability.H 없음")
          + "<h3>앙상블</h3>" + _p2_table(rel_e, _RELIAB_ORDER, "reliability.ens 없음") + "</section>")
    ...
    s9 = ('<section class="panel" id="s9"><h2>⑨ 결정론 · PIT · 누수 카나리 · spec</h2>'
          '<div class="note">같은 관측에 두 번 적합하면 θ 는 비트 동일해야 하고, filter_probabilities 는 잘라도 '
          '앞부분이 그대로여야 합니다(PIT). 누수 카나리: smoothed 가 filter 보다 AUC 가 높아야 필터가 실제로 '
          '돌고 있다는 뜻입니다.</div>'
          + _p3_kv(det, "determinism 없음")
          + ("<h3>자기검사</h3>" + _p3_kv(st) if st else "")
          + "<h3>spec</h3>" + _p3_kv(spec)
          + ("<h3>경고</h3>" + _warn_list(warns) if warns else "")
          + "</section>")

    body = (head + nav + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9
            + _p3_honesty_section("s10", kill_power_text=kill_power_line(s)))
    _write_html(out_html, "국면 엔진·등록부 (Phase 3) — market-risk-lab", body)


# ------------------------------------------------------------------
# 주간 리포트 — 트랙레코드 (docs/track_record.html)  §8.4 지평 규칙 × §8.5 패널
# ------------------------------------------------------------------
_TRACK_SECTIONS = ["① 지평 규칙", "② 확률·상태", "③ 전환·비중", "④ 변동성·밴드", "⑤ skill", "⑥ 규칙 vs 보유",
                   "⑦ 킬 카운트다운", "⑧ 경보·D10", "⑨ 등록부", "⑩ 정직 문구"]
_TRACK_PANEL_TITLES = {
    "i_p_level": "(i) 평균 p · p > 0.30 비율",
    "ii_state_occupancy": "(ii) 상태 점유",
    "iii_switching": "(iii) 결정 전환 · 비중 변경",
    "iv_exposure": "(iv) 비중 분포",
    "v_vol_forecast": "(v) 변동성 예측",
    "vi_coverage": "(vi) 밴드 포함률",
    "vii_skill": "(vii) Brier · BSS + 같은 길이 창의 백테스트 분포",
    "viii_relative": "(viii) 규칙 vs 보유",
    "ix_kill": "(ix) 킬 카운트다운 · 중간 점검",
    "x_alarms": "(x) 경보 · D10 불일치",
    "xi_registry": "(xi) 등록부 — 멤버별 라이브 · 불일치 폭 · 신선 블록",
}
...
def render_track_record(track_p3: dict, summary_p3: dict, out_html: Path, charts: dict[str, bytes]) -> None:
    ...
    tags = [f'<span class="tag">지평 <b>{_esc(_hz_stage(hz))}</b></span>',
            f'<span class="tag">라이브 시작 <b>{_esc(str(t.get("live_start") or "미시작"))}</b></span>',
            f'<span class="tag">채점 <b>{_fmt(t.get("n_scored"), "n")}행 (창 {_fmt_p3(t.get("n_eff"), "n_eff")})</b></span>',
            f'<span class="tag">경과 <b>{_fmt(t.get("months_elapsed"), "n")}개월</b></span>']
    if t.get("holdout_locked"):
        tags.append(f'<span class="tag">홀드아웃 <b>{_esc(HOLDOUT_START)}~ 잠김</b></span>')
    al = [a for a in (t.get("alarms") or []) if isinstance(a, dict)]
    if al:
        tags.append(f'<span class="tag warn">경보 {len(al)}건</span>')
    head = _p3_head("market-risk-lab · Phase 3 · 트랙레코드 · 드리프트 · 킬룰",
                    "라이브 장부 vs 백테스트 — 일기이지 판결이 아니다",
                    "모든 라이브 통계는 장부에서만 다시 계산합니다(새 백테스트에서 가져오지 않습니다). "
                    "판정일은 36개월(1차)과 8회 또는 60개월(2차)이며, 그 전에는 어떤 판정어도 쓰지 않습니다.",
                    tags, acc, eff, t.get("deploy_sizing"))
    nav = _p3_nav(_TRACK_SECTIONS)

    # ① 지평 규칙 (§8.4) — 이 페이지의 나머지가 따르는 규칙
    s1 = ('<section class="panel" id="s1"><h2>① 지평별 표시 규칙 (§8.4 — 코드가 강제)</h2>'
          + _hz_block(hz)
          + '<div class="note">n_eff &lt; 6 이면 Brier·skill 숫자를 회색으로도 표시하지 않고, 판정일(36개월) 전에는 '
            '판정어를 쓰지 않습니다. 이 규칙은 mrl/track.py 의 horizon_stage·may_show 가 단일 원천입니다.</div>'
          + "</section>")

    def _sec(sec_id: str, title: str, keys: list[str], extra: str = "") -> str:
        body = ""
        for k in keys:
            blk = panel.get(k)
            body += f"<h3>{_esc(_TRACK_PANEL_TITLES.get(k, k))}</h3>"
            body += (_panel_kv(_hz_redact(blk, hz)) if isinstance(blk, dict)
                     else '<div class="note">패널 자료 없음</div>')
        return f'<section class="panel" id="{_esc(sec_id)}"><h2>{_esc(title)}</h2>{body}{extra}</section>'

    s2 = _sec("s2", "② 확률 수준과 상태 점유", ["i_p_level", "ii_state_occupancy"])
    s3 = _sec("s3", "③ 결정 전환 · 비중 변경 · 비중 분포", ["iii_switching", "iv_exposure"])
    ...
    s4 = _sec("s4", "④ 변동성 예측과 밴드 포함률", ["v_vol_forecast", "vi_coverage"],
              _img(charts.pop("bins_live", None),
                   "확률 구간 표 — 백테스트 실현 비율 vs 라이브(Wilson, n_eff 기준).",
                   "Bins: backtest vs live")
              + "<h3>확률 구간 표 (§7 A — 전체; 라이브 열은 라이브 시작 후)</h3>"
              + _p3_table(bin_rows, _P3_BIN_ORDER, "scenarios.bins 없음")
              + "<h3>결정 상태 표 (§7 B — 상태는 범주라 풀링하지 않는다; 얇은 칸은 회색)</h3>"
              + _p3_table(state_rows, _P3_STATE_ORDER, "scenarios.states 없음")
              + (f'<div class="tag bad">회색(단독 표시 금지): {_esc(", ".join(grey_rows))} — '
                 f'n_eff &lt; {SCENARIO_P3["min_n_eff"]:.0f}</div>' if grey_rows else "")
              + f'<div class="note">구간 표는 n·n_eff(= n/{SCENARIO_P3["n_eff_div"]})·Wilson 없이 표시하지 '
                f'않으며, n_eff &lt; {SCENARIO_P3["min_n_eff"]:.0f} 인 구간은 위에서부터 풀링합니다(코드가 강제). '
                '라이브 비율이 백테스트 Wilson 구간 밖이면 \'보정 드리프트\' 로 표시합니다(D8 과 별개).</div>')

    # ⑤ skill — 지평이 막으면 숫자 대신 사유
    vii = _hz_redact(_sub(panel, "vii_skill"), hz)
    skill_head = f"<h3>{_esc(_TRACK_PANEL_TITLES['vii_skill'])}</h3>"
    if horizon_may_show(hz, "bss"):
        skill_body = skill_head + _panel_kv(vii) + _img(
            charts.pop("bss_vs_hist", None),
            "라이브 BSS 와 같은 길이 과거 창의 분포 — 한 점이 분포 안에 있는지 봅니다.",
            "Live BSS vs backtest window distribution")
    else:
        skill_body = (skill_head + f'<div class="note">{_esc(_hz_reason(hz, "bss"))}</div>'
                      '<div class="note">지금 보이는 것은 파이프라인이 매일 돌고 기록이 맞게 쌓인다는 것뿐입니다 — '
                      '계수·경보·비중 경로·에피소드 수·밴드 적중·변동성 log-MAE 는 위 섹션에 있습니다.</div>')
    s5 = f'<section class="panel" id="s5"><h2>⑤ Brier · skill</h2>{skill_body}</section>'

    s6 = _sec("s6", "⑥ 규칙 vs 보유 (같은 길이 과거 분포 옆에)", ["viii_relative"])
    ...
    s7 = ('<section class="panel" id="s7"><h2>⑦ 킬 카운트다운 · 중간 점검</h2>'
          f'<div class="v-act">판정: {verdict_txt}</div>'
          + f"<h3>{_esc(_TRACK_PANEL_TITLES['ix_kill'])}</h3>" + _panel_kv(ix)
          + ("<h3>킬 기록</h3>" + _p3_kv(kill_rows) if kill_rows else "")
          + (f'<div class="note">돌파일: {_esc(", ".join(str(x) for x in (kill.get("breach_dates") or [])))}</div>'
             if kill.get("breach_dates") else "")
          + _img(charts.pop("kill_countdown", None), "킬 카운트다운 — 에피소드 X/8 · 경과 Y/36개월(상한 60).",
                 "Kill rule countdown")
          + '<div class="note">12·24개월 중간 점검은 숫자만 보여 주고 킬할 수 없습니다(§7 의 3년 하한). '
            '킬은 sticky 이며 복귀는 새 장부 항목 + 킬 후 ≥ 12개월 + 전체 라이브 창의 CI 하한 &gt; 0 으로만 '
            '가능합니다(자동 복귀 없음).</div>'
          + f'<div class="note">{_esc(kill_power_line(s))}</div>'
          + "</section>")

    # ⑧ 경보·D10
    s8 = ('<section class="panel" id="s8"><h2>⑧ 드리프트 경보 (D1~D11) · D10 재현 불일치</h2>'
          + f"<h3>{_esc(_TRACK_PANEL_TITLES['x_alarms'])}</h3>"
          + _p3_table(al, _P3_ALARM_ORDER, "발화한 경보 없음")
          + f'<div class="note">D10 재현 불일치 {_fmt(t.get("replay_mismatch_count"), "n")}건 · '
            'D7(최근 20세션 |Δp| &gt; 0.01)은 daily 를 exit 1 로 실패시킵니다(커밋 없음).</div>'
          + _img(charts.pop("alarms_timeline", None), "경보 타임라인 — 코드별 발화 시점.", "Alarm timeline")
          + "<h3>등록된 임계 (§8.2 · P3_DRIFT — 변경은 번호 붙인 장부 항목으로만)</h3>"
          + '<div class="note">임계는 2003~24 참조분포의 p5/p95 에서 왔으므로 그 기록에 대해 post hoc 입니다. '
            '라이브 자료로 재조정하지 않습니다. 경보는 플래그·기록일 뿐 자동 재조정을 하지 않습니다.</div>'
          ...
          + ("<h3>기록</h3>" + _warn_list(notes) if notes else "")
          + "</section>")
    ...
    s9 = ('<section class="panel" id="s9"><h2>⑨ 등록부 — 멤버별 라이브 · 불일치 폭 · 신선 블록</h2>'
          + f"<h3>{_esc(_TRACK_PANEL_TITLES['xi_registry'])}</h3>"
          + _p3_table(mem_rows, ["member", "live_start", "n", "n_eff", "brier", "bss_clim", "verdict"],
                      "멤버 라이브 기록 없음")
          + (f'<div class="note">{_esc(_hz_reason(hz, "bss"))}</div>' if not horizon_may_show(hz, "bss") else "")
          + "<h3>불일치 폭</h3>" + _p3_kv(_sub(xi, "disagreement"), "불일치 기록 없음")
          + f'<div class="note">신선 블록: {_esc(str(xi.get("fresh_blocks") or t.get("fresh_blocks") or "라이브 시작 전"))} '
            f'(필요 {ENSEMBLE_P3["fresh_blocks_min"]}개)</div>'
          + "</section>")

    body = (head + nav + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9
            + _p3_honesty_section("s10", kill_power_text=kill_power_line(s)))
    _write_html(out_html, "트랙레코드 (Phase 3) — market-risk-lab", body)
    ...
    if oos is not None:
        try:
            ...
            series = [(("p_p2", "prob_dd5_20", "p_m3"), "#e6eaf2", "p2 (production)"),
                      (("p_m1", "p2_p_m1"), "#60a5fa", "M1 (calibrated VIX)"),
